...
Les chemins `MALT_EXPORTS_DIR` et `OUTPUT_DIR` du notebook sont **relatifs à la racine du pipeline texte** (dossier `text/`, qui contient `topic_eval/`), pas au répertoire `notebooks/`, grâce à `topic_eval.paths.resolve_repo_path`.
