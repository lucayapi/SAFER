...
# Exemple :
# sbatch jobft_supcon.sh \
#   --target pred_label \
#   --grid_output_root models_research/grid_search_supcon_qwen_06_pred_label \
#   --base_model_name Qwen/Qwen3-Embedding-0.6B \
...
if [[ -z "$GRID_OUTPUT_ROOT" ]]; then
  GRID_OUTPUT_ROOT="models_research/grid_search_supcon_qwen_06_${TARGET}"
fi
...
[ -f "ftemb_script_supcon.py" ] || { echo "[ERROR] ftemb_script_supcon.py introuvable"; exit 1; }
[ -f "ftemb_supcon.py" ] || { echo "[ERROR] ftemb_supcon.py introuvable"; exit 1; }
...
srun python -u ftemb_script_supcon.py \
  --target pred_label \
  --grid_output_root models_research/grid_search_embeddinggemma_pred_label \
